...
class Submission extends Model
{
    ...
    public function openBlockingComments()
    {
        return $this->comments()->where('level','must_fix')->where('status','open');
    }

    public function recomputeStatus(): void
    {

        $hasBlocking = $this->comments()->where('level','must_fix')->where('status','open')->exists();
        if ($hasBlocking) {
            if ($this->status !== 'revisao_solicitada') {
                $this->status = 'revisao_solicitada';
                $this->save();
            }
            return;
        }

        $hasOpen = $this->comments()->where('status','open')->exists();
        if ($hasOpen) {
    ...
    {
        $cases = collect(self::STATUS_ORDER)->map(fn ($st, $i) => "WHEN '{$st}' THEN {$i}")->implode(' ');
        return $q->orderByRaw("CASE status {$cases} ELSE 999 END")->orderByDesc('updated_at');
    }
    ...
    {
        return $this->isDraft() || $this->isRevRequested();
    }

    public function isLocked(): bool
    {
        return in_array($this->status, [self::ST_ACCEPTED, self::ST_REJECTED], true);
    }
}
