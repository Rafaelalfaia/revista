   ...
   public function index(Request $r)
    {
        ...
        // Admin (ou quem tiver a permissão) pode ver as submissões de um autor específico
        $authorId  = (int) ($r->integer('author_id') ?: $viewer->id);
        if ($authorId !== $viewer->id) {
            abort_unless(
                ($viewer->hasRole('Admin') || $viewer->can('submissions.viewAny')),
                403
            );
        }
        ...
        $from       = $r->date('from');
        $to         = $r->date('to');
        $dateField  = in_array($r->get('date_field'), ['created_at','submitted_at'], true)
            ? $r->get('date_field') : 'created_at';
        ...
        // Para preencher selects do filtro (se quiser usar a mesma view do autor)
        $categories = Category::orderBy('name')->get(['id','name']);
        $authors    = User::whereIn('id', Submission::select('user_id')->distinct())
                        ->orderBy('name')->get(['id','name']);

        return view('autor.submissions.index', compact(
            'subs','categories','authors','authorId','categoryId','from','to','dateField'
        ));
    }
    ...
    public function store(Request $r, SubmissionStructureService $svc)
    {
        $data = $r->validate([
            ...
            // categorias na criação (opcional)
        ...
        $submission = Submission::create([
            ...
            'status'        => \App\Models\Submission::ST_DRAFT,
        ]);
        ...
        return redirect()->route('autor.submissions.wizard', $submission)
            ->with('ok', 'Submissão criada. Preencha as seções do wizard.');
    }
    ...
    public function wizard(Submission $submission)
    {
        $sections = $submission->sections()
            ->roots()
            ->with(['assets','children.assets','children.children.assets'])
            ->get();
        ...
        $primaryCategoryId = $submission->categories()
    ...
    public function updateSection(Request $r, Submission $submission, SubmissionSection $section)
    {
        ...
        if ($nav === 'prev' && $prevId) {
            return redirect()->route('autor.submissions.section.edit', [$submission, $prevId])
                ->with('ok', 'Seção salva. Voltando para a anterior.');
        }
        if ($nav === 'next' && $nextId) {
            return redirect()->route('autor.submissions.section.edit', [$submission, $nextId])
                ->with('ok', 'Seção salva. Indo para a próxima.');
        }
    ...
    SubmissionStructureService $svc
    ) {
        $oldType = $submission->tipo_trabalho;

        // Tipos válidos (se existir o map no Model)
        $validTypes = array_keys(Submission::TYPE_LABELS ?? []);

        // Se o form veio do bloco "Categorias" (context=categories), exigimos category_id
        ...
        // --- Metadados básicos (quando não é o form de categorias) ---
        if (!$isCategoriesContext) {
            // keywords → array
            ...
            // Se o tipo mudou, sincroniza esqueleto de seções
        ...
        // --- Atualização da CATEGORIA (única e obrigatória) ---
        $categoryId = (int) $r->input('category_id');

        if (method_exists($submission, 'syncCategories')) {
            // Usa helper se você já tiver; garante 1 categoria e primaria = true
            $submission->syncCategories([$categoryId], $categoryId);
        } else {
            // Fallback direto na relação pivot
    ...
    public function assetsStore(Request $r, Submission $submission)
    {
        $data = $r->validate([
            ...
            'file'       => ['nullable','file','max:51200'], // até 50MB
    ...
    public function uploadAsset(Submission $submission, Request $request)
    {
        ...
        $disk = config('filesystems.disks.private') ? 'private' : config('filesystems.default','local');
        $dir  = "submissions/{$submission->id}/assets";

        $path = $mime = null; $size = 0;
        ...
        if ($request->filled('chart_json')) {
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                return back()
                    ->withErrors(['chart_json' => 'JSON inválido: '.json_last_error_msg()])
                    ->withInput();
            }
            ...
            $type = 'figure'; // gráfico conta como figura
        }
        // 2) Upload de arquivo normal
        elseif ($file = $request->file('file')) {
            ...
            return back()
                ->withErrors(['file' => 'Envie um arquivo ou cole um JSON de gráfico.'])
                ->withInput();
        }
    ...
    public function submit(Request $r, Submission $submission, AutoAssignService $auto)
    {
        ...
        $filled = $submission->sections()
            ...
            ->count();
        if ($filled < 1) $errors[] = 'Pelo menos uma seção deve estar preenchida.';
        ...
        if (!empty($errors)) {
            return back()->withErrors($errors)->with('error', 'Revise os itens antes de submeter.');
        }
        ...
        return redirect()->route('autor.submissions.index')
            ->with('ok', $assigned > 0
                ...
                : 'Submissão enviada para triagem. (Nenhum revisor elegível encontrado para a categoria primária)');
    }
    ...
    private function bootstrapSectionsInternal(Submission $sub): void
    {
        $map = match ($sub->tipo_trabalho) {
            ...
            Submission::TP_REV_SIST => [
                ...
                'Métodos (protocolo, busca, critérios, extração, risco de viés)',
                'Resultados (estudos incluídos e síntese)',
                'Discussão',
                'Conclusões',
                'Agradecimentos',
                'Referências',
