...
@php
  ...
  if (isset($reviewers) && is_array($reviewers) && count($reviewers)) {
      $reviewersList = collect($reviewers);
  } elseif ($submission->reviews ?? null) {
      $reviewersList = ($submission->reviews)->pluck('reviewer.name')->filter();
  } else {
      $reviewersList = collect();
  }
  ...
  <div class="reader-header-card">
    <div class="reader-header-top">
      <div class="reader-title-block">
        <div class="reader-label">Admin · Submissão #{{ $submission->id }}</div>
    ...
    <div class="reader-header-bottom">
      ...
      @if($submission->created_at)
        <span><span>Criado em</span><span>{{ $submission->created_at->format('d/m/Y H:i') }}</span></span>
      @endif
      @if($submission->updated_at)
        <span><span>Última atualização</span><span>{{ $submission->updated_at->format('d/m/Y H:i') }}</span></span>
      @endif
      @if($slug)
        <span><span>Slug:</span><span>{{ $slug }}</span></span>
      @endif
    </div>
  </div>
